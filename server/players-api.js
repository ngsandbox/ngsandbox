"use strict";
var Player = require("./mongoose/player-schema");
function regPlayersApi(app) {
    /* Create or save */
    app.put('/api/player/:id?', function (req, res) {
        ...
        if (req.params.id) {
            ...
            var upsertData = newPlayer.toObject();
            // Delete the _id property, otherwise Mongo will return a "Mod on _id not allowed" error
            delete upsertData._id;
            // Do the upsert, which works like this: If no Contact document exists with 
            // _id = contact.id, then create a new doc using upsertData.
            // Otherwise, update the existing doc with upsertData
            Player.update({ _id: req.params.id }, upsertData, { upsert: true }, function (err) { return savePlayer(newPlayer, res, err); });
        }
        else {
            newPlayer.save(function (err) { return savePlayer(newPlayer, res, err); });
        }
    });
...
exports.regPlayersApi = regPlayersApi;
